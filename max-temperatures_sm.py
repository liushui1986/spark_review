...
# Filters the RDD to keep only records where the entryType is "TMAX".
maxTemps = parsedLines.filter(lambda x: "TMAX" in x[1])

# MAP to (stationID, temperature) pairs
# Transforms the RDD to only include the station ID and temperature, ignoring the entryType (all "TMAX")
stationTemps = maxTemps.map(lambda x: (x[0], x[2]))

# Find the minimum temperature per Station
# Aggregates the temperatures by station ID, keeping only the maximum temperature for each station.
